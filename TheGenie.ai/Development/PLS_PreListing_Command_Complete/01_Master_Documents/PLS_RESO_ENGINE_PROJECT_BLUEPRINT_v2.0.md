...
21. [Ecosystem Integrations - Complete Technical Stack](#21-ecosystem-integrations---complete-technical-stack) ⭐ NEW
22. [Exhibits Catalog - All Project Documents](#22-exhibits-catalog---all-project-documents) ⭐ NEW
23. [Reference Documents (Legacy)](#23-reference-documents-legacy---see-exhibits-catalog-above)
...
```json
{
  ...
  "photos": [{ "url": "https://...", "displayOrder": 1, "isPrimary": true }],
  "areaId": 407559
}
```
...
const routes: Routes = [
  {
    ...
    children: [
      ...
      {
        path: "initiate/:plsNumber",
        component: ListingCommandInitiateComponent,
      },
    ],
  },
];
...
## 21. ECOSYSTEM INTEGRATIONS - COMPLETE TECHNICAL STACK

### 21.1 Integration Overview

PLS is a **feature addition** to the existing TheGenie.ai platform, not a standalone application. This section provides a complete 360-degree view of all ecosystem components PLS integrates with, their technical implementation, and how they work together.

**Critical Understanding:** PLS developers must work **within** existing infrastructure, not build new infrastructure.

### 21.2 Paisley AI Integration - Pre-Listing Focus Enhancement

**Status:** ✅ **REPLACES** existing Paisley Pre-Listing Focus prompt  
**Enhancement:** PLS significantly enhances the existing Pre-Listing Focus functionality

#### Current Paisley Pre-Listing Focus (ChatStartTypeId=3)

**What Exists:**

- Paisley AI service with ChatStartTypeId=3 (Pre-Listing Focused)
- Uses Assessor data (TitleData.dbo.AttomDataAssessor) - NOT MLS data
- Generates property descriptions for pre-listing scenarios
- Currently accessible via Paisley UI

**How PLS Enhances It:**

1. **Integrated Workflow:** PLS replaces the standalone Paisley prompt with a complete listing creation workflow
2. **Auto-Generation:** Description auto-generates when property is selected (no manual prompt needed)
3. **Context-Rich:** PLS provides Listing Data + Area Data to Paisley for richer descriptions
4. **UI Integration:** Description appears in PLS form with "Edit" button (seamless UX)
5. **Listing Lifecycle:** Generated description becomes part of the PLS listing record
...
**📚 Complete Documentation:** See [Exhibit A.1](#exhibit-a1-paisley-integration) - [PLS_MASTER_SPECIFICATION_v3.md](./PLS_MASTER_SPECIFICATION_v3.md) Section 2.2

---

### 21.3 GenieCloud Integration - XML Generation & Render Process

**Status:** ✅ **CRITICAL** - XML is the foundation of all asset generation  
**Contract:** `CONTRACT_PLS_to_GenieCloud_v6.1.md`

#### XML Generation Process

**Step 1: PLS Generates XML**

- PLS builds XML structure per Contract v6.1
- Maps `MlsListing.dbo.Listing` columns → XML elements
- Includes agent data, area data, property data
- Validates XML structure before sending

**Step 2: POST to GenieCloud API**

- Endpoint: `POST https://cloud-api.thegenie.ai/api/render`
- Payload: XML string + render settings
- Response: Render job ID + collection URL

**Step 3: GenieCloud XSL Processing**

- GenieCloud receives XML
- Applies XSL template (e.g., `pls-hollywood.xsl`)
- Transforms XML → HTML structure
- Applies theme (e.g., `compass dark`)

**Step 4: Puppeteer Rendering**

- GenieCloud uses Puppeteer to render HTML
- Generates PNG/PDF assets
- Stores in S3 bucket (`genie-cloud`, `us-west-1`)
- Creates collection page

**Step 5: Collection Control**

- GenieCloud returns collection URL
- PLS stores collection URL in `pls_tracking.collection_url`
- User can manage assets via Genie Collection Editor (existing tool)
- Assets accessible via collection page

#### XSL Template System

**Templates:**

- `pls-hollywood.xsl` - Landing page template
- `lc-prop-post-03.xsl` - Social graphic template
- `lc-prop-post-01-vip.xsl` - VIP social template

**Theme Application:**

- Theme: `compass` (CSS class)
- Hue: `dark` or `light` (CSS class)
- Applied via XSL: `<body class="compass dark">`

**Render Process:**

```
XML → XSL Transform → HTML → Puppeteer → PNG/PDF → S3 Storage
```
...
- Standalone tool for managing collections
- PLS will integrate into Dashboard UI
- Users can:
...
```
https://cloud.thegenie.ai/genie-collection/{collection-id}
```

**Collection Page:**

- Displays all assets in collection
- Organized by asset type (landing pages, social graphics, etc.)
- Responsive design
- Shareable link

**📚 Complete Documentation:** See [Exhibit A.2](#exhibit-a2-geniecloud-contract) - [CONTRACT_PLS_to_GenieCloud_v6.1.md](./CONTRACT_PLS_to_GenieCloud_v6.1.md)

---

### 21.4 Listing Command Integration - Circle Prospecting Automation

**Status:** ✅ **CRITICAL** - PLS uses existing Listing Command workflow  
**PropertyCastTypeId:** `4` (PLS)

#### Integration Pattern

**PLS follows EXACT same pattern as Listing Command:**

1. PLS listing created with `PropertyCastTypeId=4`
2. INSERT into `ListingCommandQueue` table
3. Listing Command service processes queue
4. Circle prospecting automation (SMS to farm area)
5. Engagement Center captures leads
6. Versium data append (automatic)
...
- Listing Command service processes `PropertyCastTypeId=4` same as `PropertyCastTypeId=1`
- No code changes needed in Listing Command service
- PLS listings appear in Listing Command UI

#### UI Integration

**Reuse Existing Component:**

- `ListingCommandInitiateComponent` (existing Angular component)
- Route: `/pls/initiate/{plsNumber}`
- Same UI, same workflow, different PropertyCastTypeId

**Workflow:**

1. User creates PLS listing
2. User clicks "Start Campaign" button
3. System routes to `ListingCommandInitiateComponent` with `plsNumber` parameter
4. Component loads PLS listing data
5. User configures campaign (same as Listing Command)
6. Campaign starts (SMS, lead capture, etc.)

**📚 Complete Documentation:** See [Exhibit A.3](#exhibit-a3-listing-command-integration) - [PLS_MASTER_SPECIFICATION_v3.md](./PLS_MASTER_SPECIFICATION_v3.md) Section 4

---

### 21.5 TitleGenie Integration - Property Data Pre-Population

**Status:** ✅ **CRITICAL** - Primary data source for PLS  
**Database:** `TitleData.dbo.AttomDataAssessor`

#### Data Source

**Table:** `TitleData.dbo.AttomDataAssessor`

- **318 fields** of property data
- Assessor records (not MLS)
- Updated via Attom SFTP feed
...
- User enters address in PLS form
- System calls `POST /api/Data/GetPropertiesFromPlaceKey`
- Google Places API geocodes address
- System queries TitleData by address or APN
...
WHERE ParcelNumberFormatted = @APN
...
- System compares TitleData vs Historical MLS data
- Flags conflicts (sqft, beds/baths differences)
- User reviews and resolves conflicts

**Step 4: Form Pre-Population**

- PLS form auto-fills with TitleData values
- Conflicts marked with asterisk (\*)
- User can edit any field

#### Field Mapping

**TitleData → MlsListing Mapping:**

- `ParcelNumberFormatted` → `APN`
- `StreetNumber` + `StreetName` → `DisplayAddress`
- `Bedrooms` → `Bedrooms`
- `BathroomsFull` → `BathroomsTotal`
- `AreaBuilding` → `Sqft`
- `YearBuilt` → `YearBuilt`
- `LotSizeSquareFeet` → `LotSqft`

**📚 Complete Documentation:** See [Exhibit A.4](#exhibit-a4-titlegenie-integration) - [TITLEDATA_TO_MLSLISTING_FIELD_MAPPING_ANALYSIS_v1.md](./TITLEDATA_TO_MLSLISTING_FIELD_MAPPING_ANALYSIS_v1.md)

---

### 21.6 Complete Ecosystem Architecture

```
┌─────────────────────────────────────────────────────────────────────────┐
│                    THEGENIE.AI ECOSYSTEM (360° VIEW)                     │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                           │
│  ┌──────────────┐    ┌──────────────┐    ┌──────────────┐               │
│  │ TITLE GENIE  │    │   PAISLEY    │    │  ENGAGEMENT  │               │
│  │              │    │              │    │    CENTER    │               │
│  │ • Attom Data │───▶│ • AI Content │    │              │               │
│  │ • 318 Fields │    │ • ChatStart3 │    │ • Lead Capture│              │
│  │ • Pre-Pop    │    │ • Enhances   │    │ • UTM Track  │               │
│  │              │    │   Pre-Listing│    │ • Versium    │               │
│  └──────┬───────┘    └──────┬───────┘    └──────┬───────┘               │
│         │                    │                    │                       │
│         ▼                    ▼                    ▼                       │
...
│  │  │              │  │ LAYER        │  │ LAYER        │  │           │
│  │  │ • Database   │─▶│ • API        │─▶│ • UI         │  │           │
│  │  │ • Stored     │  │ • XML Gen    │  │ • Forms      │  │           │
│  │  │   Procedures │  │ • Validation │  │ • Uploads    │  │           │
│  │  └──────────────┘  └──────────────┘  └──────────────┘  │           │
│  │                                                           │           │
│  │  • MlsId=777      • PropertyCastTypeId=4                │           │
│  │  • StatusTypeID 6/14  • Listing Command Integration      │           │
...
│  │                     GENIE CLOUD                            │           │
│  │  • XSL Templates  • Puppeteer Renderer  • S3 Storage      │           │
│  │  • XML → HTML     • PNG/PDF Gen         • Collections     │           │
│  │  • Collection Editor (Existing Tool)                      │           │
│  └───────────────────────────────────────────────────────────┘           │
│                              │                                           │
│                              ▼                                           │
│  ┌───────────────────────────────────────────────────────────┐           │
│  │                  LISTING COMMAND                           │           │
│  │  • PropertyCastTypeId=4  • Circle Prospecting              │           │
│  │  • SMS Campaign          • Lead Capture                    │           │
│  └───────────────────────────────────────────────────────────┘           │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
## 22. EXHIBITS CATALOG - ALL PROJECT DOCUMENTS

**DRA-2026 Compliant:** This master document catalogs all related documents as exhibits. No orphaned documents.

### Exhibit A: Core Specifications

#### <a id="exhibit-a1-paisley-integration"></a>Exhibit A.1: Paisley Integration

- **Document:** [PLS_MASTER_SPECIFICATION_v3.md](./PLS_MASTER_SPECIFICATION_v3.md)
- **Location:** `01_Master_Documents/`
- **Purpose:** Complete Paisley ChatStartTypeId=3 integration specification
- **Key Sections:** Section 2.2 (Paisley Chat Types), Section 17 (Title Genie Integration)

#### <a id="exhibit-a2-geniecloud-contract"></a>Exhibit A.2: GenieCloud Contract

- **Document:** [CONTRACT_PLS_to_GenieCloud_v6.1.md](./CONTRACT_PLS_to_GenieCloud_v6.1.md) (also in [11_Contracts/](../11_Contracts/CONTRACT_PLS_to_GenieCloud_v6.1.md))
- **Location:** `01_Master_Documents/` and `11_Contracts/`
- **Purpose:** Complete XML data contract, XSL generation, render process, collection system
- **Key Sections:** Section 4 (XML Data Structure), Section 12 (Collection System), Section 13 (Genie Collection Editor)

#### <a id="exhibit-a3-listing-command-integration"></a>Exhibit A.3: Listing Command Integration

- **Document:** [PLS_MASTER_SPECIFICATION_v3.md](./PLS_MASTER_SPECIFICATION_v3.md) Section 4
- **Location:** `01_Master_Documents/`
- **Purpose:** PropertyCast integration, queue system, workflow
- **Key Sections:** Section 4 (PropertyCast Integration)

#### <a id="exhibit-a4-titlegenie-integration"></a>Exhibit A.4: TitleGenie Integration

- **Document:** [TITLEDATA_TO_MLSLISTING_FIELD_MAPPING_ANALYSIS_v1.md](./TITLEDATA_TO_MLSLISTING_FIELD_MAPPING_ANALYSIS_v1.md)
- **Location:** `01_Master_Documents/`
- **Purpose:** Complete field mapping from TitleData to MlsListing
- **Key Content:** 318 field mapping, conflict resolution

### Exhibit B: Database & Infrastructure

#### Exhibit B.1: Database Schema

- **Document:** [PLS_DATABASE_SCHEMA_RELATIONAL_v1.md](./PLS_DATABASE_SCHEMA_RELATIONAL_v1.md)
- **Location:** `01_Master_Documents/`
- **Purpose:** Complete normalized database schema
- **Key Content:** All tables, relationships, stored procedures

#### Exhibit B.2: Schema Visual Diagram

- **Document:** [PLS_SCHEMA_VISUAL_DIAGRAM_NORMALIZED_v3.md](../06_Infrastructure/PLS_SCHEMA_VISUAL_DIAGRAM_NORMALIZED_v3.md)
- **Location:** `06_Infrastructure/`
- **Purpose:** Visual representation of database schema with joins
- **Key Content:** ER diagrams, table relationships

#### Exhibit B.3: Schema SQL Scripts

- **Document:** [PLS_SCHEMA_EXTENSIONS_NORMALIZED_v3.sql](../02_Scripts/PLS_SCHEMA_EXTENSIONS_NORMALIZED_v3.sql)
- **Location:** `02_Scripts/`
- **Purpose:** Complete SQL implementation scripts
- **Key Content:** CREATE TABLE statements, stored procedures, master data

#### Exhibit B.4: Permissions & Roles

- **Document:** [PLS_PERMISSION_ROLE_INTEGRATION_v1.md](../06_Infrastructure/PLS_PERMISSION_ROLE_INTEGRATION_v1.md)
- **Location:** `06_Infrastructure/`
- **Purpose:** Complete permission system specification
- **Key Content:** Permission types, role matrix, authorization patterns

### Exhibit C: Development & Onboarding

#### Exhibit C.1: Dev Team Onboarding Package

- **Document:** [PLS_DEV_TEAM_ONBOARDING_PACKAGE_v1.md](./PLS_DEV_TEAM_ONBOARDING_PACKAGE_v1.md)
- **Location:** `01_Master_Documents/`
- **Purpose:** Complete 360-degree platform view for development team
- **Key Content:** Tech stack, infrastructure, constraints, integration points

#### Exhibit C.2: Dev Team Preparation Checklist

- **Document:** [PLS_DEV_TEAM_PREPARATION_CHECKLIST_v1.md](./PLS_DEV_TEAM_PREPARATION_CHECKLIST_v1.md)
- **Location:** `01_Master_Documents/`
- **Purpose:** Checklist of what documentation exists vs what's missing
- **Key Content:** Preparation tasks, priorities, 3-phase approach

#### Exhibit C.3: Platform Documentation Master

- **Document:** [PLS_PLATFORM_DOCUMENTATION_MASTER_v1.md](./PLS_PLATFORM_DOCUMENTATION_MASTER_v1.md)
- **Location:** `01_Master_Documents/`
- **Purpose:** Complete platform picture (source code, stored procedures, roles, permissions, infrastructure)
- **Key Content:** Visual tools, eRealtor documentation structure, legacy constraints

### Exhibit D: Project Management

#### Exhibit D.1: Project Reset Master

- **Document:** [PLS_PROJECT_RESET_MASTER_v1.md](./PLS_PROJECT_RESET_MASTER_v1.md)
- **Location:** `01_Master_Documents/`
- **Purpose:** Project reset, document organization, sprint schedule
- **Key Content:** Document index, feature prioritization, 3-sprint schedule

#### Exhibit D.2: Blueprint Gap Analysis

- **Document:** [PLS_BLUEPRINT_GAP_ANALYSIS_v1.md](./PLS_BLUEPRINT_GAP_ANALYSIS_v1.md)
- **Location:** `01_Master_Documents/`
- **Purpose:** Comparison with eRealtor spec, identifies missing sections
- **Key Content:** Critical gaps, medium priority gaps, completion status

### Exhibit E: Contracts & Agreements

#### Exhibit E.1: GenieCloud Contract (Active)

- **Document:** [CONTRACT_PLS_to_GenieCloud_v6.1.md](./CONTRACT_PLS_to_GenieCloud_v6.1.md) (also in [11_Contracts/](../11_Contracts/CONTRACT_PLS_to_GenieCloud_v6.1.md))
- **Location:** `01_Master_Documents/` and `11_Contracts/`
- **Purpose:** Shared contract between PLS and GenieCloud teams
- **Status:** ✅ Active - v6.1

#### Exhibit E.2: GenieCloud Contract (Deprecated)

- **Documents:** `CONTRACT_PLS_to_GenieCloud_v1.md` through `v5.md`
- **Location:** `11_Contracts/`
- **Status:** ❌ Deprecated - Superseded by v6.1

### Exhibit F: Analysis & Discovery

#### Exhibit F.1: GenieCloud XML Mapping

- **Document:** [PLS_GENIECLOUD_XML_MAPPING_v1.md](./PLS_GENIECLOUD_XML_MAPPING_v1.md)
- **Location:** `01_Master_Documents/`
- **Purpose:** Map existing GenieCloud collection prototype to XML format
- **Key Content:** Collection → XML mapping, production examples

#### Exhibit F.2: 3-Layer Gap Analysis

- **Document:** [PLS_3_LAYER_GAP_ANALYSIS_v1.md](./PLS_3_LAYER_GAP_ANALYSIS_v1.md)
- **Location:** `01_Master_Documents/`
- **Purpose:** Gap analysis of 3-layer architecture implementation
- **Key Content:** Data layer gaps, function layer gaps, interface layer gaps

### Exhibit G: UI & Wireframes

#### Exhibit G.1: Wireframe Specifications

- **Document:** [PLS_WIREFRAME_SPECIFICATIONS_v1.md](./PLS_WIREFRAME_SPECIFICATIONS_v1.md)
- **Location:** `01_Master_Documents/`
- **Purpose:** Complete UI wireframe specifications
- **Key Content:** Component designs, user flows, interactions

### Exhibit H: Scripts & Implementation

#### Exhibit H.1: Database Setup Scripts

- **Documents:** Multiple SQL scripts
- **Location:** `02_Scripts/`
- **Purpose:** Database implementation scripts
- **Key Files:**
  - [PLS_SCHEMA_EXTENSIONS_NORMALIZED_v3.sql](../02_Scripts/PLS_SCHEMA_EXTENSIONS_NORMALIZED_v3.sql) - Complete schema
  - [PLS_DATABASE_MASTER_DATA_v3.sql](../02_Scripts/PLS_DATABASE_MASTER_DATA_v3.sql) - Master data inserts
  - [PLS_STORED_PROCEDURES_COMPLETE_v1.sql](../02_Scripts/PLS_STORED_PROCEDURES_COMPLETE_v1.sql) - Stored procedures

#### Exhibit H.2: Deployment Scripts

- **Documents:** PowerShell and Python scripts
- **Location:** `02_Scripts/`
- **Purpose:** Deployment automation
- **Key Files:**
  - [deploy_to_s3_v1.py](../02_Scripts/deploy_to_s3_v1.py) - S3 deployment
  - [FIX_WEBCONFIG_CONNECTION_STRINGS.ps1](../02_Scripts/FIX_WEBCONFIG_CONNECTION_STRINGS.ps1) - Configuration fixes

### Exhibit I: Infrastructure Documentation

#### Exhibit I.1: Integration Discovery

- **Document:** [PLS_INTEGRATION_DISCOVERY_v1.md](../06_Infrastructure/PLS_INTEGRATION_DISCOVERY_v1.md)
- **Location:** `06_Infrastructure/`
- **Purpose:** Integration point discovery and analysis
- **Key Content:** API endpoints, data flows, integration patterns

#### Exhibit I.2: Database Items Checklist

- **Document:** [PLS_DATABASE_ITEMS_CHECKLIST_v3.md](../06_Infrastructure/PLS_DATABASE_ITEMS_CHECKLIST_v3.md)
- **Location:** `06_Infrastructure/`
- **Purpose:** Checklist of all database items to create
- **Key Content:** Tables, stored procedures, indexes, constraints

### Exhibit J: Ecosystem Document Catalog ⭐ NEW

#### Exhibit J.1: Complete Ecosystem Catalog

- **Document:** [PLS_ECOSYSTEM_DOCUMENT_CATALOG_v1.md](./PLS_ECOSYSTEM_DOCUMENT_CATALOG_v1.md)
- **Location:** `01_Master_Documents/`
- **Purpose:** Complete catalog of all Paisley, Title Genie, and PLS documents
- **Key Content:** Document inventory, integration points, agent assignments, critical documents by priority

### Exhibit K: Pre-Listing Command Legacy Documents ⭐ NEW

#### Exhibit K.1: Pre-Listing Command Master Specification

- **Document:** [PLS_MASTER_SPECIFICATION_v3.md](../../Paisley/Pre.Listing.Command/Docs/PLS_MASTER_SPECIFICATION_v3.md)
- **Location:** `Paisley/Pre.Listing.Command/Docs/`
- **Purpose:** Complete PLS specification from original Pre-Listing Command project
- **Key Content:** System context, PropertyCast integration, Title Genie integration, production examples
- **Status:** ✅ Active - v3.0 (consolidated from v1 and v2)

#### Exhibit K.2: Production Reference Index

- **Document:** [MASTER_PRODUCTION_REFERENCE_INDEX_v1.md](../../Paisley/Pre.Listing.Command/Docs/MASTER_PRODUCTION_REFERENCE_INDEX_v1.md)
- **Location:** `Paisley/Pre.Listing.Command/Docs/`
- **Purpose:** Central index of all LIVE PRODUCTION examples (THE GOLD STANDARD)
- **Key Content:**
  - Dainelle Scott Listing Command Proof Kit (Collection ID: 15a521b8-3fbf-4042-bce3-58e378cd9a52)
  - 10037 Rebecca Place PLS Hollywood MVP (Live URL)
  - Theme references, user accounts, source code locations
- **Critical:** ⚠️ **ALWAYS CHECK THESE FIRST** before building/rendering any assets

#### Exhibit K.3: PLS Hollywood MVP SOP

- **Document:** [SOP_PLS_Hollywood_MVP_Creation_v1.md](../../Paisley/Pre.Listing.Command/Docs/SOP_PLS_Hollywood_MVP_Creation_v1.md)
- **Location:** `Paisley/Pre.Listing.Command/Docs/`
- **Purpose:** Step-by-step SOP for creating PLS-Hollywood Private Listing pages (MVP process)
- **Key Content:** Required data fields, agent information, area information, step-by-step process

#### Exhibit K.4: Collection Editor Admin Integration

- **Document:** [COLLECTION_EDITOR_ADMIN_INTEGRATION_v1.md](../../Paisley/Pre.Listing.Command/Docs/COLLECTION_EDITOR_ADMIN_INTEGRATION_v1.md)
- **Location:** `Paisley/Pre.Listing.Command/Docs/`
- **Purpose:** Genie Collection Editor integration into TheGenie dashboard
- **Key Content:** Collection Editor tool specifications, integration requirements

#### Exhibit K.5: Collection Manager Analysis

- **Document:** [COLLECTION_MANAGER_ANALYSIS_v1.md](../../Paisley/Pre.Listing.Command/Docs/COLLECTION_MANAGER_ANALYSIS_v1.md)
- **Location:** `Paisley/Pre.Listing.Command/Docs/`
- **Purpose:** Collection management system analysis
- **Key Content:** Collection system architecture, management workflows

#### Exhibit K.6: Genie Role Permission Framework

- **Document:** [GENIE_ROLE_PERMISSION_FRAMEWORK_v1.md](../../Paisley/Pre.Listing.Command/Docs/GENIE_ROLE_PERMISSION_FRAMEWORK_v1.md)
- **Location:** `Paisley/Pre.Listing.Command/Docs/`
- **Purpose:** Complete role and permission framework for Genie ecosystem
- **Key Content:** Role definitions, permission matrix, authorization patterns

#### Exhibit K.7: Workspace Context (Legacy Migration)

- **Document:** [WORKSPACE_CONTEXT_12-28-2025_v1.md](../../Paisley/Pre.Listing.Command/Discovery/WORKSPACE_CONTEXT_12-28-2025_v1.md)
- **Location:** `Paisley/Pre.Listing.Command/Discovery/`
- **Purpose:** Legacy Visio files migration and workspace organization
- **Key Content:** 97 Visio files organized into 8 categories, project context

### Exhibit L: Paisley Project Documents ⭐ NEW

#### Exhibit L.1: Paisley Master Index

- **Document:** [MASTER_INDEX_v3.md](../../Paisley/MASTER_INDEX_v3.md)
- **Location:** `Paisley/`
- **Purpose:** Central index of all Paisley project documents
- **Key Content:** Master documents, data catalogs, discovery docs, workspace memory logs

#### Exhibit L.2: Paisley Pre-Listing Data Catalog v2

- **Document:** [PAISLEY_PRELISTING_DATA_CATALOG_v2.md](../../Paisley/PAISLEY_PRELISTING_DATA_CATALOG_v2.md)
- **Location:** `Paisley/`
- **Purpose:** Complete data field catalog (500+ fields) for pre-listing intelligence
- **Key Content:** Versium seller intel, ViewAssessor_v3 (315+ fields), mortgage data, liens, HOA, AVM, flood zone
- **Critical:** ⭐⭐⭐ **CRITICAL** - Data available for PLS pre-population

#### Exhibit L.3: Paisley Project Scope Blueprint

- **Document:** [PAISLEY_PROJECT_SCOPE_BLUEPRINT_v1.md](../../Paisley/PAISLEY_PROJECT_SCOPE_BLUEPRINT_v1.md)
- **Location:** `Paisley/`
- **Purpose:** Intent-focused project agreement for Paisley Pre-Listing Suite
- **Key Content:** Customer expectations, Lawyers Title requirements, existing suite pages

#### Exhibit L.4: Paisley Complete Walkthrough

- **Document:** [PAISLEY_PRELISTING_COMPLETE_WALKTHROUGH_v1.md](../../Paisley/PAISLEY_PRELISTING_COMPLETE_WALKTHROUGH_v1.md)
- **Location:** `Paisley/`
- **Purpose:** Complete pre-listing workflow documentation
- **Key Content:** User flow, data requirements, integration points
- **Critical:** ⭐⭐⭐ **CRITICAL** - User flow reference for PLS

#### Exhibit L.5: Paisley Workspace Memory Logs

- **Documents:** Multiple workspace memory logs
- **Location:** `Paisley/`
- **Purpose:** Historical development context and decisions
- **Key Files:**
  - `WORKSPACE_MEMORY_LOG_PreListing_Command_Handoff_v1.md` - Direct PLS handoff
  - `WORKSPACE_MEMORY_LOG_PreListing_Command_v3_2025-12-25.md` - Latest PLS integration work
  - `WORKSPACE_MEMORY_LOG_TitleGenie_Paisley_Discovery_Session_2025-12-19_v2.md` - Joint discovery
- **Critical:** ⭐⭐⭐ **CRITICAL** - Read these for integration context

### Exhibit M: Title Genie Project Documents ⭐ NEW

#### Exhibit M.1: Title Genie Master Compilation

- **Document:** [TITLEGENIE_MASTER_COMPILATION_v1.md](../../TitleGenie/TITLEGENIE_MASTER_COMPILATION_v1.md)
- **Location:** `TitleGenie/`
- **Purpose:** Complete compilation of all Title Genie findings
- **Key Content:** Goals, features, roadmap, GTM plan, pricing, products, Paisley integration
- **Critical:** ⭐⭐⭐ **CRITICAL** - Complete Title Genie context

#### Exhibit M.2: Title Genie Discovery Compilation

- **Document:** [TITLEGENIE_DISCOVERY_COMPILATION_v1.2.md](../../TitleGenie/Discovery/TITLEGENIE_DISCOVERY_COMPILATION_v1.2.md)
- **Location:** `TitleGenie/Discovery/`
- **Purpose:** Complete discovery findings for Title Genie
- **Key Content:** Discovery questions, answers, feature requirements

#### Exhibit M.3: Title Genie MVP Roadmap GTM Plan

- **Document:** [TITLEGENIE_MVP_ROADMAP_GTM_PLAN_v2.md](../../TitleGenie/TITLEGENIE_MVP_ROADMAP_GTM_PLAN_v2.md)
- **Location:** `TitleGenie/`
- **Purpose:** Roadmap and go-to-market plan for Title Genie
- **Key Content:** Phase 1-3 roadmap, GTM strategy, success metrics

### Exhibit N: Agent Collaboration & Management ⭐ NEW

#### Exhibit N.1: Agent Coordination Master

- **Document:** [PLS_AGENT_COORDINATION_MASTER_v1.md](../AgentCollaboration/PLS_AGENT_COORDINATION_MASTER_v1.md)
- **Location:** `AgentCollaboration/`
- **Purpose:** Single source of truth for agent coordination
- **Key Content:** 5 role definitions, status tracking, collaboration protocols, quick start guide

#### Exhibit N.2: Project Manager Role Definition

- **Document:** [AGENT_ROLE_PROJECT_MANAGER_v1.md](../AgentCollaboration/AGENT_ROLE_PROJECT_MANAGER_v1.md)
- **Location:** `AgentCollaboration/`
- **Purpose:** Project Manager (JR) role definition and responsibilities
- **Key Content:** Daily workflow, escalation process, first priorities

#### Exhibit N.3: Individual Agent Role Definitions

- **Documents:** 5 role definition files
- **Location:** `AgentCollaboration/`
- **Purpose:** Individual role definitions for each specialized agent
- **Key Files:**
  - `AGENT_ROLE_DATABASE_SPECIALIST_v1.md`
  - `AGENT_ROLE_BACKEND_API_SPECIALIST_v1.md`
  - `AGENT_ROLE_FRONTEND_UI_SPECIALIST_v1.md`
  - `AGENT_ROLE_XML_INTEGRATION_SPECIALIST_v1.md`
  - `AGENT_ROLE_DEVOPS_SPECIALIST_v1.md`

---

## 23. REFERENCE DOCUMENTS (Legacy - See Exhibits Catalog Above)

**Note:** This section maintained for backward compatibility. All documents now cataloged in [Section 22: Exhibits Catalog](#22-exhibits-catalog---all-project-documents).

### Master Documents

| Document                               | Version | Location                                   | Status    |
| -------------------------------------- | ------- | ------------------------------------------ | --------- |
| **CONTRACT_PLS_to_GenieCloud**         | 6.1     | `01_Master_Documents/` and `11_Contracts/` | ✅ Active |
| **PLS_MASTER_SPECIFICATION**           | 3.0     | `01_Master_Documents/`                     | ✅ Active |
| **PLS_DEV_TEAM_ONBOARDING_PACKAGE**    | 1.0     | `01_Master_Documents/`                     | ✅ Active |
| **PLS_DEV_TEAM_PREPARATION_CHECKLIST** | 1.0     | `01_Master_Documents/`                     | ✅ Active |

### Implementation Specs

| Document                           | Version | Location               | Status    |
| ---------------------------------- | ------- | ---------------------- | --------- |
| **PLS_DATABASE_SCHEMA_RELATIONAL** | 1.0     | `01_Master_Documents/` | ✅ Active |
| **PLS_WIREFRAME_SPECIFICATIONS**   | 1.0     | `01_Master_Documents/` | ✅ Active |
| **PLS_GENIECLOUD_XML_MAPPING**     | 1.0     | `01_Master_Documents/` | ✅ Active |

### Analysis Documents

| Document                                  | Version | Location               | Status    |
| ----------------------------------------- | ------- | ---------------------- | --------- |
| **TITLEDATA_TO_MLSLISTING_FIELD_MAPPING** | 1.0     | `01_Master_Documents/` | ✅ Active |
| **PLS_BLUEPRINT_GAP_ANALYSIS**            | 1.0     | `01_Master_Documents/` | ✅ Active |
| **PLS_3_LAYER_GAP_ANALYSIS**              | 1.0     | `01_Master_Documents/` | ✅ Active |
...
| ------- | ----------------------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **2.1** | **01/13/2026 11:30 PM** | **JR (Project Manager)** | **ECOSYSTEM DOCUMENT EXPANSION** - Added Exhibits J-N to Section 22 (Exhibits Catalog): (J) Ecosystem Document Catalog, (K) Pre-Listing Command Legacy Documents (7 exhibits including Master Specification v3, Production Reference Index, MVP SOP, Collection Editor docs), (L) Paisley Project Documents (5 exhibits including Master Index, Data Catalog v2, Complete Walkthrough, Workspace Memory Logs), (M) Title Genie Project Documents (3 exhibits including Master Compilation, Discovery Compilation, MVP Roadmap), (N) Agent Collaboration & Management (3 exhibits including Agent Coordination Master, Project Manager Role, Individual Role Definitions). All discovered documents from Paisley/Pre.Listing.Command, Paisley workspace, and Title Genie now cataloged. Knowledge base fully updated with ecosystem context. |
| **2.0** | **01/13/2026 10:30 PM** | **JR (Project Manager)** | **MAJOR VERSION - ECOSYSTEM INTEGRATION & EXHIBITS CATALOG** - Added Section 21 (Ecosystem Integrations) with complete technical stack documentation for Paisley, GenieCloud, Listing Command, and TitleGenie. Added Section 22 (Exhibits Catalog) cataloging all project documents per DRA-2026. Enhanced integration sections emphasize: (1) Paisley replaces/enhances Pre-Listing Focus, (2) GenieCloud XSL generation and render process, (3) GenieCloud collection control system, (4) Listing Command connectivity, (5) TitleGenie integration. All documents now cataloged as exhibits with no orphaned documents. Updated Table of Contents.                                                                                                                                                                                        |
| 1.15    | 01/10/2026 3:15 AM      | Danny (Dev Lead)         | **ROLLBACK PROCEDURE FIX - DLL.CONFIG CRITICAL** - Updated Section 14 (Deployment Prompt Beta) to include `bin\Smart.Dashboard.dll.config` in backup/rollback procedures. Documented why rollback failed (DLL.config was missing from backup). Added complete backup and rollback script examples. Added critical lesson learned: DLL.config must be included in all backups because it's loaded at application startup and contains connection strings. If DLL.config is not restored, authentication will fail even if Web.config is correct. This is why rollback didn't work - DLL.config wasn't backed up or restored.                                                                                                                                                                                                                 |
