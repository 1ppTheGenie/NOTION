-- ============================================================================
-- COMPETITION COMMAND FULL COST REPORT - Multi-Month
-- Version: 1.0 | Generated: 2025-12-14
...
;WITH 
-- Base CC Areas (from SmsReportSendQueue)
CCBase AS (
    SELECT 
        srq.AreaId,
        MAX(srq.AspNetUserId) AS AspNetUserId,
...
-- Delivered count (ResponseCode = 1 success)
DeliveredCounts AS (
    SELECT 
        srq.AreaId,
        SUM(v.Count) AS Delivered
    FROM SmsReportSendQueue srq WITH (NOLOCK)
    INNER JOIN ViewSmsQueueSendSummary v ON v.SmsReportSendQueueId = srq.SmsReportSendQueueId
    WHERE srq.CreateDate >= @Nov_Start AND srq.CreateDate < @Nov_End
      AND srq.UtmSource = 'Competition Command'
      AND v.ResponseCode = 1
    GROUP BY srq.AreaId
),
-- Clicks from GenieLead (distinct leads created from clicks)
Clicks AS (
    SELECT gl.AreaId, COUNT(DISTINCT gl.GenieLeadId) AS Clicks
    FROM dbo.GenieLead gl WITH (NOLOCK)
    WHERE gl.CreateDate >= @Nov_Start AND gl.CreateDate < @Nov_End
      AND gl.AreaId IS NOT NULL
    GROUP BY gl.AreaId
),
-- CTA Events (Submitted/Verified)
CTAEvents AS (
    SELECT gl.AreaId,
    ...
    WHERE gl.CreateDate >= @Nov_Start AND gl.CreateDate < @Nov_End
      AND gl.AreaId IS NOT NULL AND ltt.Tag LIKE 'Cta%'
    GROUP BY gl.AreaId
),
-- Opt Outs
OptOuts AS (
    ...
    INNER JOIN dbo.GenieLeadTag glt WITH (NOLOCK) ON glt.GenieLeadId = gl.GenieLeadId
    WHERE gl.CreateDate >= @Nov_Start AND gl.CreateDate < @Nov_End
      AND gl.AreaId IS NOT NULL AND glt.LeadTagTypeId = 51
    GROUP BY gl.AreaId
),
-- Agent Notify (extract GenieLeadId from NotificationQueue.CustomData)
AgentNotifyRaw AS (
...
AgentNotify AS (
    SELECT gl.AreaId, 
        COUNT(DISTINCT anr.NotificationQueueId) AS Agent_Notify,
        SUM(anr.TwilioCost) AS Agent_Notify_Cost
...
-- Audience Twilio Cost (from TwilioMessage via SmsReportSendQueue)
AudienceTwilioCost AS (
    SELECT srq.AreaId, SUM(ABS(ISNULL(tm.Price, 0))) AS Twilio_Cost
    FROM SmsReportSendQueue srq WITH (NOLOCK)
    INNER JOIN SmsQueue sq WITH (NOLOCK) ON sq.SmsReportSendQueueId = srq.SmsReportSendQueueId
    LEFT JOIN TwilioMessage tm WITH (NOLOCK) ON tm.Sid = sq.TwilioSmsSid
    WHERE srq.CreateDate >= @Nov_Start AND srq.CreateDate < @Nov_End
      AND srq.UtmSource = 'Competition Command'
    GROUP BY srq.AreaId
)
...
SELECT 
    ...
    CASE WHEN cc.Msgs_Sent > 0 THEN CAST(ISNULL(dc.Delivered,0) * 100.0 / cc.Msgs_Sent AS DECIMAL(5,2)) ELSE 0 END AS Success_Pct,
    ISNULL(clk.Clicks, 0) AS Clicks,
    CASE WHEN cc.Msgs_Sent > 0 THEN CAST(ISNULL(clk.Clicks,0) * 100.0 / cc.Msgs_Sent AS DECIMAL(5,2)) ELSE 0 END AS CTR_Pct,
    ...
    ISNULL(an.Agent_Notify_Cost, 0) AS Agent_Notify_Cost,
    ISNULL(op.Opt_Outs, 0) AS Opt_Outs,
    CASE WHEN cc.Msgs_Sent > 0 THEN CAST(ISNULL(op.Opt_Outs,0) * 100.0 / cc.Msgs_Sent AS DECIMAL(5,2)) ELSE 0 END AS Opt_Out_Pct,
    ISNULL(atc.Twilio_Cost, 0) AS Twilio_Cost
FROM CCBase cc
...
;WITH 
CCBase AS (
    SELECT srq.AreaId, MAX(srq.AspNetUserId) AS AspNetUserId,
...
SELECT 
    ...
    COALESCE(pno.FriendlyName, va.Name, CAST(cc.AreaId AS VARCHAR)) AS Area_Name,
    cc.AreaId,
    cc.Campaigns, cc.Msgs_Sent, ISNULL(dc.Delivered, 0),
...
SELECT 
    ...
    '$' + CAST(Twilio_Cost AS VARCHAR) AS Twilio_Cost
FROM #CCReport
ORDER BY 
    CASE Month 
...
SELECT 
    ...
    '$' + CAST(SUM(Twilio_Cost) AS VARCHAR) AS Total_Twilio_Cost
FROM #CCReport
GROUP BY Month
ORDER BY 
    CASE Month 
